...
void System::MainLoopDkp()
{
   ...
   while (1)
   {
      ...
      if (!InputCmd.empty() && InputCmd[0] == "dkpconfig")
      {
      ...
      else if (!InputCmd.empty() && InputCmd[0] == "dkptool")
      {
      ...
      else if (!InputCmd.empty() && InputCmd[0] == "cd")
      {
         chdir(InputCmd[1].c_str());
         continue;
      }
      else if (!InputCmd.empty() && InputCmd[0] == "exit")
      {
         std::cout << Color::RED << "Exit" << std::endl;
         exit(0);
      }
